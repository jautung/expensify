...
public struct PieChartRow : View {
        ...
        GeometryReader { geometry in
            ZStack{
                ForEach(0..<self.slices.count){ i in
                    PieChartCell(rect: geometry.frame(in: .local), startDeg: self.slices[i].startDeg, endDeg: self.slices[i].endDeg, index: i, backgroundColor: self.backgroundColor, accentColor: lightenColorN(color: self.accentColor, n: i))
    ...
    func lightenColorN(color: Color, n: Int) -> Color {
        var newColor: Color = color
        for _ in 0..<n { newColor = lightenColor(color: newColor) }
        return newColor
    }
    
    func lightenColor(color: Color) -> Color {
